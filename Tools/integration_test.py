...
try:
 ...
 call('stop');call('timer','markers','2,4,6','--seconds');call('meditate','--reset');wait_audio('living');time.sleep(8.1)
 ...
 call('play');wait_audio('living');time.sleep(.5);check(call('status')['elapsed_seconds']>paused+.4,'Resume preserves time')
 ...
 call('solo',imported['id']);call('play');check(imported['id'] in wait_audio(imported['id'])['audio_playing_ids'],'Import playback')
 ...
 # Capture the owned app before any failure cleanup, including the complete
 # import/history/preview/quit sequence that was reported to hang elsewhere.
