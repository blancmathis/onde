...
try:
 ...
 check(g['orchestra_samples']==0,'Pure electronic scene does not load unused acoustic samples')
 ...
 check(g['orchestra_samples']==76,'All verified acoustic samples are available when an acoustic scene needs them')
 ...
 check(g['orchestra_samples']==0,'Returning to a synthetic scene releases its acoustic scene references')
 ...
 check(g['orchestra_samples']==76,'Shared acoustic bank is retained correctly after cancellation and resume')
 call('silence');time.sleep(1.4);s=call('status')
 check(s['status']!='playing' and s['active_sound_ids']==[],'Silent Focus pauses the session')
 time.sleep(.6);frozen=call('status')
 check(abs(frozen['elapsed_seconds']-s['elapsed_seconds'])<.05,'Silent Focus freezes elapsed time')
 check(abs(frozen['today_seconds']-s['today_seconds'])<.05,'Silent Focus freezes daily accounting')
