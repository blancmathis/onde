...
with tempfile.TemporaryDirectory(prefix='onde-espace-', dir=os.environ.get('TMPDIR')) as folder:
    ...
    assert receipt['count']==19 and receipt['muted'] and not receipt['playing'], receipt
    assert receipt['timer_driver_checks']==3 and receipt['software_surface_frames']==2
    assert len(list(output.glob('*-native.png')))==19
    assert (output/'surface-0.png').read_bytes() != (output/'surface-8.png').read_bytes()
    assert (output/'software-surface-0.png').read_bytes() != (output/'software-surface-8.png').read_bytes()
